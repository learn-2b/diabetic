...
import serial  # مكتبة pyserial للتواصل مع Arduino
import time

# إعداد الاتصال التسلسلي مع Arduino
arduino = serial.Serial('COM3', 9600)  # غيّر COM3 إلى المنفذ الصحيح في جهازك
time.sleep(2)  # الانتظار قليلاً حتى يتهيأ الاتصال
...
def predict(image_path):
    ...
    # إرسال النتيجة إلى Arduino
    arduino.write(str(predicted_class).encode())  # إرسال رقم الفئة إلى Arduino
    return result_text
